...
describe("Lottery", () => {
  ...
  describe("PickWinner", () => {
    ...
    it.skip("Calculate winner - getRandomNumber()", async () => {
      ...
      const calculatedRandomNumber =
        ethers.BigNumber.from(ethers.utils.solidityKeccak256(["address", "uint256"], [await lottery.owner(), currentBlock.timestamp]));
      console.log(`calculated random number: ${calculatedRandomNumber}`);
      expect(randomNumber).to.equal(calculatedRandomNumber);

      const calculatedWinnerIndex =
        ethers.BigNumber.from(ethers.utils.solidityKeccak256(["address", "uint256"], [await lottery.owner(), currentBlock.timestamp])).mod(5);
    ...
    it("Calculate winner - getRandomNumberV2()", async () => { // truffle에서도 안되더니 여기서도 안되네
      const lotteryId = await lottery.lotteryId();
      console.log(`lotteryId: ${lotteryId}`);
      ...
      const calculatedRandomNumber = ethers.BigNumber.from(ethers.utils.solidityKeccak256(["uint256", "uint256", "address[]"],
        [currentBlock.difficulty, currentBlock.timestamp, [signers[1].address, signers[2].address, signers[3].address, signers[4].address, signers[5].address]]));
      console.log(`calculated random number: ${calculatedRandomNumber}`);

      const calculatedWinnerIndex = ethers.BigNumber.from(ethers.utils.solidityKeccak256(["uint256", "uint256", "address[]"],
        [currentBlock.difficulty, currentBlock.timestamp, [signers[1].address, signers[2].address, signers[3].address, signers[4].address, signers[5].address]]))
        .mod(5);
      console.log(`calculated winner index: ${calculatedWinnerIndex}`);
      expect(winner).to.equal(signers[calculatedWinnerIndex.add(1)].address);
    ...
    it.skip("Calculate winner - getRandomNumberV3()", async () => {
      ...
      const calculatedRandomNumber = 
        ethers.BigNumber.from(ethers.utils.solidityKeccak256(["bytes32", "uint256"], [currentBlock.parentHash, currentBlock.timestamp]));
      console.log(`calculate random number: ${calculatedRandomNumber}`);

      const calculatedWinnerIndex = calculatedRandomNumber.mod(5);
      console.log(`calculated winner index: ${calculatedWinnerIndex}`);
      expect(winner).to.equal(signers[calculatedWinnerIndex.add(1)].address);
    });
  });
